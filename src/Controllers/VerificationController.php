...
class VerificationController extends Controller
{
    ...
    public function confirm(Request $request)
    {
        if (! $token = $request->input('token')) {
            return $this->makeConfirmView(trans('login-verification::login-verification.missing_token'));
        }

        if (! $email = $request->input('email')) {
            return $this->makeConfirmView(trans('login-verification::login-verification.missing_email'));
        }

        try {
            $token = decrypt($token);
        } catch (Throwable $e) {
            return $this->makeConfirmView(trans('login-verification::login-verification.invalid_token'));
        }
        ...
        if ($token['email'] !== $email) {
            return $this->makeConfirmView(trans('login-verification::login-verification.invalid_email'));
        }
        ...
        if ($token['timestamp'] + $expiresIn < time()) {
            return $this->makeConfirmView(trans('login-verification::login-verification.token_expired'));
        }
        ...
        return $this->makeConfirmView(trans('login-verification::login-verification.confirm_successfully'));
    }
